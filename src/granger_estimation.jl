...
function est_sig_covs(data::AbstractArray{Float64,2}, order::Int)::Array{Float64,2}
    ...
    for k = 1:order
        @views Covs[:, (1:nchan).+k*nchan] =
            covariance(data[k+1:nsamples, :], data[1:nsamples-k, :])
    end
    return Covs
end
...
function mvar_est(Covs::Array{Float64,2})
    ...
    for k = 1:order
        ...
        for L = 1:k-1
            ...
            ARB[:, S4], ARF[:, S3] =
                (ARB[:, S4] - ARB[:, S2] * ARF[:, S3], ARF[:, S3] - ARF[:, S2] * ARB[:, S4])
        end
...
function granger_jackknife(
    ...
    Threads.@threads for i = 1:nsegments
        ...
        SegmentC =
            vcat(view(data, 1:(i-1)*seglen, :), view(data, (i+1)*seglen.+1:nsamples, :))
...
"""
function granger_aic(data::Array{Float64,2}, seglen::Int, order_range::UnitRange{Int64})
...
"""
function granger_bic(data::Array{Float64,2}, seglen::Int, order_range::UnitRange{Int64})
